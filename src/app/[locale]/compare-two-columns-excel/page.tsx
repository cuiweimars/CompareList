...
export default function CompareTwoColumnsExcelPage() {
  const t = useTranslations("compareTwoColumnsExcel");
      ...
      <main className="max-w-[960px] mx-auto px-4 py-12">
        ...
        <div className="mt-16 space-y-8">
          <div className="glass rounded-xl p-6">
            <h2 className="font-semibold text-lg mb-3 font-[family-name:var(--font-sora)]">{t('whyUse.heading')}</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {[0, 1, 2, 3].map((i) => (
                <div key={i} className="bg-surface-alt/30 rounded-lg p-3">
                  <h3 className="font-medium text-sm mb-1">{t(`whyUse.${i}.title`)}</h3>
                  <p className="text-xs text-text-secondary">{t(`whyUse.${i}.desc`)}</p>
                </div>
              ))}
            </div>
          </div>
